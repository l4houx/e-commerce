...
sf-meili-import: ## Import all indices.
	$(SYMFONY_CONSOLE) meili:import
.PHONY: sf-meili-import

sf-meili-import-indices: ## Choose what indices to re-index by passing the index name.
	$(SYMFONY_CONSOLE) meili:import --indices=posts,recipes
.PHONY: sf-meili-import-indices

sf-meili-create: ## Create all indexes (based on the existing models).
	$(SYMFONY_CONSOLE) meili:create
.PHONY: sf-meili-create

sf-meili-create-indices: ## Create the post and recipes indexes.
	$(SYMFONY_CONSOLE) meili:create --indices=posts,recipes
.PHONY: sf-meili-create-indices

sf-meili-clear: ## Clear all indices.
	$(SYMFONY_CONSOLE) meili:clear
.PHONY: sf-meili-clear

sf-meili-clear-indices: ## Choose what indices to clear by passing the index name.
	$(SYMFONY_CONSOLE) meili:clear --indices=posts,recipes
.PHONY: sf-meili-clear-indices

sf-meili-delete-indices: ## Remove the index.
	$(SYMFONY_CONSOLE) meili:delete --indices=posts,recipes
.PHONY: sf-meili-delete-indices
#---------------------------------------------#
